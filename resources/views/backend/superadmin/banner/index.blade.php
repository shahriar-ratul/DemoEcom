    ...
    <div class="card card-custom card-custom gutter-t">
        <div class="card-header py-3">
            <div class="card-title">
                <h3 class="card-label">All Banner List</h3>
            </div>
            <div class="card-toolbar">

                <a href="{{ route('superadmin.banner.create') }}" class="btn btn-primary font-weight-bolder">
            ...
            <table class="table table-separate table-head-custom table-checkable" id="kt_datatable3">
                ...
                @foreach ($banners as $key=>$banner)
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                <div class="symbol symbol-40 symbol-sm flex-shrink-0">
                                    <img class="" src="{{asset('banner')}}/{{$banner->image}}" alt="Slider Image">
                                </div>
                            </div>
                        </td>
                        ...
                        <td>
                            <a class="btn btn-info" href="{{ route('superadmin.banner.edit',$banner->id) }}">
                                <i class="fas fa-edit"></i>
                            </a>
                            <a class="btn btn-danger text-light" onclick="deleteTag({{ $banner->id }})">
                                <i class="fas fa-trash-alt"></i>
                            </a>
                            <form id="delete-form-{{ $banner->id }}" action="{{ route('superadmin.banner.destroy',$banner->id) }}" method="POST" style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
                        </td>
